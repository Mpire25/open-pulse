...
export const SLEEP_TOOL_DESCRIPTION =
  'Read sleep sessions by their end or wake date over an explicit date range. "Last night" means the sleep session ending today; do not subtract a day. "The night before last" means the session ending yesterday. localStartTime and localEndTime are already converted for user-facing clock-time answers; never reinterpret them as UTC. Summary mode returns timing, duration, efficiency and stage totals without raw stage segments. Use detailed mode only when interruption or stage detail is central to the question.'
